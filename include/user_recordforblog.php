...
include('../include/connectforblog.php');
session_start();
...
} else if (basename($_SERVER['PHP_SELF']) != 'index.php' && basename($_SERVER['PHP_SELF']) != 'login.php' && basename($_SERVER['PHP_SELF']) != 'register.php' && basename($_SERVER['PHP_SELF']) != 'updatepassword.php' && basename($_SERVER['PHP_SELF']) != 'blog.php' && strpos($_SERVER['PHP_SELF'], '/blog/') === false) {
 $title = 'Error';
